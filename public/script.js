(function () {
  ...
  if (menuBtn && links) {
    ...
    links.addEventListener("click", (e) => {
      const t = e.target;
      if (t && t.tagName === "A") links.classList.remove("open");
    });
  }
  ...
  class Slider {
    constructor(root) {
      ...
      this.prevBtn = $(".prev", root);
      this.nextBtn = $(".next", root);
    ...
    go(n) {
      ...
      this.lock = true;
      this.update();
      setTimeout(() => (this.lock = false), 620);
      this.resetAuto();
    }
    updateDots() {
    ...
    startProgress() {
      ...
      this.bar.style.animation = "progress 5s linear";
    }
    bindSwipe() {
      let startX = 0,
        dx = 0,
      ...
      const onMove = (e) => {
        ...
        dx = x - startX;
        this.slidesWrap.style.transform = `translateX(calc(${
          -this.index * 100
        }% + ${dx}px))`;
      };
      const onUp = () => {
  ...
  if (orbs.length) {
    window.addEventListener("mousemove", (e) => {
      ...
      orbs.forEach((o, i) => {
        const f = i === 0 ? 12 : i === 1 ? 8 : 15;
        o.style.transform = `translate3d(${dx * f}px, ${dy * f}px, 0)`;
      });
    });
  }
  ...
  if (lightbox) {
    ...
    lightbox.addEventListener("click", (e) => {
      if (e.target === lightbox) window.__lightbox.close();
    });
    $$("[data-lightbox]").forEach((a) => {
      a.addEventListener("click", (e) => {
        e.preventDefault();
        const thumb = $(".thumb", a);
        const imgEl = thumb ? $("img", thumb) : null;
        if (imgEl && imgEl.src) {
          window.__lightbox.open(imgEl.src);
          return;
        }
  ...
  if (canvas) {
    ...
    function step() {
      ...
      for (const s of stars) {
        s.x += s.z * 0.12;
        if (s.x > w + 10) s.x = -10;
        const twinkle = (1 + Math.sin(t * 6 + s.tw)) / 2;
        ctx.beginPath();
        ctx.fillStyle = s.c;
        ctx.globalAlpha = 0.35 + twinkle * 0.35;
